...
if not OPENROUTER_API_KEY:
    raise RuntimeError(
        ...
        "Set it locally via .env or in AWS Secrets Manager for production."
    )
...
def get_technical_skills():
    return json.dumps({
        ...
        "mlops": ["Docker", "Kubernetes", "MLflow", "Kubeflow", "Apache Airflow", "DVC"],
        "cloud": ["Google Cloud Platform (ML services and compute)", "AWS (App Runner, ECR, Secrets Manager)"],
...
def get_projects():
    return json.dumps({
        ...
        "digital_twin": {
            "status": "Live",
            "description": "AI-powered digital twin chatbot that represents me professionally using tool calling and Gradio, containerized and deployed on AWS App Runner",
            "tech": ["OpenAI SDK", "OpenRouter", "Gradio", "Python", "Docker", "AWS App Runner", "Terraform", "GitHub Actions"],
